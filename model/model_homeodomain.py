...
from .dynamics import EnergyDynamicsProtein, EnergyPredictor
...
class PESLA_Homeodomain(nn.Module):
    def __init__(self, conf):
        super().__init__()
        
        # Model args
        code_dim, K = conf.PESLA_Homeodomain.code_dim, conf.PESLA_Homeodomain.K
        self.conf = conf
        
        # VQ-VAE
        self.encoder, self.decoder = AE(conf)
        self.codebook = CodeBook(K, code_dim, conf.PESLA_Homeodomain.softmax_temperature, init='uniform')
        
        # Energy Predictor
        self.energyNet = EnergyPredictor(conf.PESLA_Homeodomain.code_dim)
    ...
    def Energy(self, x):
        z_e_x = self.encoder(x)
        # z_q_x_st, z_q_x, x_code_idx = self.codebook.straight_through(z_e_x, greedy=True) # (B, D)
        # return self.energyNet(z_q_x_st)
        return self.energyNet(z_e_x)

    def forward(self, x, test=False):
        # x: (B, C)
        z_e_x = self.encoder(x) # (B, D)
        greedy = self.conf.PESLA_Homeodomain.greedy_train or test
    ...
    def predict(self, x):
        ...
        h1, p1_logit = self.dynamics.evolve_one_step(x_code_idx, lag=self.conf.Homeodomain.lag)
        return h1, p1_logit
    ...
    def sample(self, init_state, n_samples):        
        ...
        sample_traj, sample_traj_code_idx, sample_traj_P = [init_state.unsqueeze(0)], [code_idx.unsqueeze(0)], [P0]
        ...
        sample_traj = torch.cat(sample_traj, dim=0).permute(1, 0, 2).cpu().detach().numpy() # (B, n_samples, C)
        sample_traj_code_idx = torch.cat(sample_traj_code_idx, dim=0).permute(1, 0).cpu().detach().numpy() # (B, n_samples)
        sample_traj_P = torch.cat(sample_traj_P, dim=0).permute(1, 0).unsqueeze(0).cpu().detach().numpy() # (B, n_samples, N)
    ...
    def fit(self):
        batch_size = self.conf.homeodomain_phase1.batch_size
        # Dataset
        ...
        batch_size = self.conf.homeodomain_phase2.batch_size
    ...
    def fit_phase1(self, train_loader, val_loader):
        ...
        optimizer = torch.optim.Adam(
            ...
            lr=self.conf.homeodomain_phase1.lr
        )
        scheduler = ExponentialLR(optimizer, gamma=self.conf.homeodomain_phase1.lr_decay)
        
        train_loss_list, val_loss_list, hit_num_list = [], [], []
        for epoch in range(1, self.conf.homeodomain_phase1.max_epoch+1):
            ...
            print(f'\rEpoch[{epoch}/{self.conf.homeodomain_phase1.max_epoch}] nll={loss_recons.item():.4f} vq={loss_vq.item():.4f} commit={loss_commit.item():.4f} energy={loss_energy.item():.4f}', end='')
            train_loss_list.append([epoch, train_loss / len(train_loader)])
            
            if epoch % self.conf.homeodomain_phase1.lr_step == 0:
                scheduler.step()
            
            if epoch % self.conf.homeodomain_phase1.val_interval == 0:
                self.eval()
                ...
                print(f'\nEpoch[{epoch}/{self.conf.homeodomain_phase1.max_epoch}] val loss={val_loss/len(val_loader):.4f} | hit num={hit_num}/{self.conf.PESLA_Homeodomain.K}')
                ...
                plt.figure(figsize=(12, 6))
                plt.subplot(1, 2, 1)
                plt.scatter(np.array(xy_true)[:,0], np.array(xy_true)[:,1], s=1)
                plt.xlabel('x')
                plt.ylabel('y')
                plt.title('True')
                plt.subplot(1, 2, 2)
                plt.scatter(np.array(xy_pred)[:,0], np.array(xy_pred)[:,1], s=1)
                plt.xlabel('x')
                plt.ylabel('y')
                plt.title('Pred')
                plt.tight_layout()
                ...
                plt.scatter(np.array(xy_pred)[:,0], np.array(xy_pred)[:,1], c='gray', s=1)
        ...
        plt.xlabel('Epoch')
        plt.ylabel(f'Hit Num (K={self.conf.PESLA_Homeodomain.K})')
        plt.tight_layout()
        plt.savefig(self.conf.log_dir+'loss_phase1.png', dpi=300)
    
    def fit_phase2(self, train_loader, val_loader):
        if os.path.exists(self.conf.log_dir+f'lag{self.conf.Homeodomain.lag}-phase2.pickle'):
            with open(self.conf.log_dir+f'lag{self.conf.Homeodomain.lag}-phase2.pickle', 'rb') as f:
            ...
            os.makedirs(self.conf.log_dir+f'val/lag{self.conf.Homeodomain.lag}-phase2/', exist_ok=True)
        ...
        print(f'Hit Code Index: {len(hit_code_idx)}/{self.conf.PESLA_Homeodomain.K}')
        ...
        self.dynamics = EnergyDynamicsProtein(self.conf, node_features, node_energy).to(self.conf.device)
        ...
        optimizer = torch.optim.Adam(
            ...
            lr=self.conf.homeodomain_phase2.lr
        )
        scheduler = ExponentialLR(optimizer, gamma=self.conf.homeodomain_phase2.lr_decay)
        
        train_loss_list, val_loss_list = [], []
        for epoch in range(1, self.conf.homeodomain_phase2.max_epoch+1):
            ...
            print(f'\rEpoch[{epoch}/{self.conf.homeodomain_phase2.max_epoch}] loss_h={loss_h.item():.4f} loss_p={loss_p.item():.4f} loss_l={loss_l.item():.4f}', end='')
            
            train_loss_list.append([epoch, train_loss / len(train_loader)])
            
            if epoch % self.conf.homeodomain_phase2.lr_step == 0:
                scheduler.step()
                
            if epoch % self.conf.homeodomain_phase2.val_interval == 0:
                self.eval()
                val_loss = 0.
                for i, (X, Y) in enumerate(val_loader):
                    H_hat, P_logit_hat = self.predict(X)
                    ...
                    val_loss += loss.item()
                print(f'\nEpoch[{epoch}/{self.conf.homeodomain_phase2.max_epoch}] val loss_h={loss_h.item():.4f} val loss_p={loss_p.item():.4f}')
                val_loss_list.append([epoch, val_loss/len(val_loader)])
                ...
                plt.tight_layout()
                plt.savefig(self.conf.log_dir+f'val/lag{self.conf.Homeodomain.lag}-phase2/p_{epoch}.png', dpi=300)
            
        # Save Phase2 model
        with open(self.conf.log_dir+f'lag{self.conf.Homeodomain.lag}-phase2.pickle', 'wb') as f:
        ...
        plt.legend(frameon=False)
        plt.tight_layout()
        plt.savefig(self.conf.log_dir+f'lag{self.conf.Homeodomain.lag}-loss_phase2.png', dpi=300)
    ...
    def test(self):
        ...
        traj_num = 1
        lag = self.conf.Homeodomain.lag
        
        # Load GT data
        gt_traj_tica = np.load('data/Homeodomain/test.npy')[-num_sample:,] # (T, C)
            
        # Sample Trajectory
        try:
            sample_traj = np.load(self.conf.log_dir+'test/sample_traj.npy')
        except:
            sample_traj, sample_traj_code_idx, sample_traj_P = self.sample(torch.tensor(gt_traj_tica[0], dtype=torch.float32).to(self.conf.device).unsqueeze(0), len(gt_traj_tica)//lag)
            sample_traj = sample_traj[0, -num_sample//lag:] # (n_sample//lag, 2)
            np.save(self.conf.log_dir+'test/sample_traj.npy', sample_traj)
        gt_traj_tica = gt_traj_tica[::lag] # (n_sample//lag, 2)
        
        # Grid Energy
        with open(f"data/Homeodomain/Homeodomain-energy.pkl", 'rb') as f:
            energy_landscape = pickle.load(f)
        sample_coords_true_energy = energy_landscape.query_energy(gt_traj_tica[:, 0], gt_traj_tica[:, 1])
        print(f'nan count = {np.isnan(sample_coords_true_energy).sum()}/{len(sample_coords_true_energy)}')
        mask = np.isfinite(sample_coords_true_energy)
        sample_coords_true_energy = sample_coords_true_energy[mask]
        
        # Pred Energy
        sample_coords_pred_energy = self.Energy(torch.tensor(gt_traj_tica, dtype=torch.float32).to(self.conf.device)).squeeze().cpu().detach().numpy()[mask]
        spearmans_rho = scipy.stats.spearmanr(sample_coords_pred_energy, sample_coords_true_energy)
        print(f'Lag={lag} | Pearson\'s ρ={spearmans_rho.correlation:.4f}')
        # RANSAC
        ransac = RANSACRegressor()
        ransac.fit(sample_coords_true_energy.reshape(-1, 1), sample_coords_pred_energy)
        ...
        # Predict
        grid_num = 5
        from utils import test_protein
        test_protein(self, grid_num, traj_num, gt_traj_tica, sample_traj, lag)